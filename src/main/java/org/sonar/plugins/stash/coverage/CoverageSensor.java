...
public class CoverageSensor implements Sensor, BatchComponent {
    ...
    public CoverageSensor(FileSystem fileSystem, ResourcePerspectives perspectives, StashPluginConfiguration config,
            ActiveRules activeRules, CoverageProjectStore coverageProjectStore) {
    ...
    public void analyse(Project module, SensorContext context) {
        ...
        for (InputFile f : fileSystem.inputFiles(fileSystem.predicates().all())) {
            ...
            if (linesToCover != null && uncoveredLines != null) {
                ...
                // So we can only report anything if the rounded value has
                // changed,
    ...
    private void addIssue(InputFile file, double coverage, double previousCoverage) {
        ...
        Issue issue = issuable.newIssueBuilder().ruleKey(CoverageRule.decreasingLineCoverageRule(file.language()))
    ...
    static String formatIssueMessage(String path, double coverage, double previousCoverage) {
        return MessageFormat.format("Line coverage of file {0} lowered from {1}% to {2}%.", path,
                formatPercentage(previousCoverage), formatPercentage(coverage));
    }
