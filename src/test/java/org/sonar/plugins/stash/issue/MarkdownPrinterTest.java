...
public class MarkdownPrinterTest {
    ...
    private PullRequestRef pr = PullRequestRef.builder().setProject("stashProject").setRepository("stashRepo").setPullRequestId(1)
            .build();

    @Before
    public void setUp() {
        Issue issueBlocker = new DefaultIssue().setKey("key1").setSeverity(Severity.BLOCKER).setMessage("messageBlocker")
                .setRuleKey(RuleKey.of("RepoBlocker", "RuleBlocker")).setLine(1);
        Issue issueCritical = new DefaultIssue().setKey("key2").setSeverity(Severity.CRITICAL).setMessage("messageCritical")
                .setRuleKey(RuleKey.of("RepoCritical", "RuleCritical")).setLine(1);
        Issue issueMajor = new DefaultIssue().setKey("key3").setSeverity(Severity.MAJOR).setMessage("messageMajor")
                .setRuleKey(RuleKey.of("RepoMajor", "RuleMajor")).setLine(1);

        report.add(issueBlocker);
        report.add(issueCritical);
        report.add(issueMajor);

        coverageIssue = new DefaultIssue().setKey("key4").setSeverity(Severity.MAJOR)
    ...
    private String printReportMarkdown(List<Issue> report, int issueThreshold) {
        return MarkdownPrinter.printReportMarkdown(pr, STASH_URL, SONAR_URL, report, issueThreshold, 40.0, 50.0,
                issuePathResolver);
    }
    ...
    private String printCoverageReportMarkdown(List<Issue> report, double projectCoverage, double previousProjectCoverage) {
        return MarkdownPrinter.printCoverageReportMarkdown(1000, "project", "repo", 1, report, STASH_URL, projectCoverage,
                previousProjectCoverage, issuePathResolver);
    }
    ...
    public void testPrintReportMarkdownMaxLength() {
        for (int i = 0; i < 5000; i++) {
            Issue issueBlocker = new DefaultIssue().setKey("key" + i).setSeverity(Severity.BLOCKER)
                    .setMessage("messageBlocker" + i).setRuleKey(RuleKey.of("RepoBlocker" + i, "RuleBlocker" + i)).setLine(1);
            Issue issueCritical = new DefaultIssue().setKey("key" + i).setSeverity(Severity.CRITICAL)
                    .setMessage("messageCritical" + i).setRuleKey(RuleKey.of("RepoCritical" + i, "RuleCritical" + i)).setLine(1);
            Issue issueMajor = new DefaultIssue().setKey("key" + i).setSeverity(Severity.MAJOR).setMessage("messageMajor" + i)
                    .setRuleKey(RuleKey.of("RepoMajor" + i, "RuleMajor" + i)).setLine(1);

            report.add(issueBlocker);
            report.add(issueCritical);
            report.add(issueMajor);

            coverageIssue = new DefaultIssue().setKey("key" + i).setSeverity(Severity.MAJOR)
                    .setRuleKey(CoverageRule.decreasingLineCoverageRule("java"))
