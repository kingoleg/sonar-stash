...
public class StashIssueReportingPostJobTest extends StashTest {
    ...
    private static final PullRequestRef pr = PullRequestRef.builder().setProject(STASH_PROJECT).setRepository(STASH_REPOSITORY)
            .setPullRequestId(STASH_PULLREQUEST_ID).build();
    ...
    public void setUp() throws Exception {
        ...
        when(stashRequestFacade.getSonarQubeReviewer(Mockito.anyString(), (StashClient) Mockito.anyObject()))
                .thenReturn(stashUser);
    ...
    public void testExecuteOn() throws Exception {
        ...
        verify(stashRequestFacade, times(0)).resetComments(eq(pr), eq(diffReport), eq(stashUser),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr), eq(report), eq(diffReport),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(1)).postAnalysisOverview(eq(pr), eq(STASH_ISSUE_THRESHOLD), eq(report),
                (StashClient) Mockito.anyObject());
    ...
    public void testExecuteOnWithReachedThreshold() throws Exception {
        ...
        verify(stashRequestFacade, times(0)).resetComments(eq(pr), eq(diffReport), eq(stashUser),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(0)).postSonarQubeReport(eq(pr), eq(report), eq(diffReport),
                (StashClient) Mockito.anyObject());
    ...
    public void testExecuteOnWithNoPluginActivation() throws Exception {
        ...
        verify(stashRequestFacade, times(0)).postSonarQubeReport(eq(pr), eq(report), eq(diffReport),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(0)).postAnalysisOverview(eq(pr), eq(STASH_ISSUE_THRESHOLD), eq(report),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(0)).approvePullRequest(eq(pr), (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(0)).resetPullRequestApproval(eq(pr), (StashClient) Mockito.anyObject());
    }
    ...
    public void testExecuteOnWithResetCommentActivated() throws Exception {
        ...
        verify(stashRequestFacade, times(1)).resetComments(eq(pr), eq(diffReport), eq(stashUser),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr), eq(report), eq(diffReport),
                (StashClient) Mockito.anyObject());
        verify(stashRequestFacade, times(1)).postAnalysisOverview(eq(pr), eq(STASH_ISSUE_THRESHOLD), eq(report),
     ...
     * 
     * @Test public void
     * testExecuteOnWithPullRequestApprovalAndNoNewIssueAndCodeCoverageEvolutionPositive
     * () throws Exception {
     ...
     * when(stashRequestFacade.extractIssueReport(projectIssues,
     * inputFileCache)).thenReturn(report);
     * 
     ...
     * 
     * when(stashRequestFacade.getCoverageReport(eq(project.getKey()),
     * eq(context), eq(inputFileCacheSensor), eq("INFO"), (SonarQubeClient)
     * Mockito.anyObject())).thenReturn(coverageReport);
     * 
     * myJob = new StashIssueReportingPostJob(config, projectIssues,
     * inputFileCache, stashRequestFacade, inputFileCacheSensor);
     * myJob.executeOn(project, context);
     * 
     * verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr),
     * eq(report), eq(diffReport), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(1)).postAnalysisOverview(eq(pr),
     * eq(STASH_ISSUE_THRESHOLD), eq(report), (StashClient)
     * Mockito.anyObject()); verify(stashRequestFacade,
     * times(1)).postCoverageReport(eq(pr), eq(diffReport), (StashClient)
     * Mockito.anyObject()); verify(stashRequestFacade,
     * times(1)).approvePullRequest(eq(pr), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(0)).resetPullRequestApproval(eq(pr),
     * (StashClient) Mockito.anyObject()); }
     */

    /*
     * @Test public void
     * testExecuteOnWithPullRequestApprovalAndNoNewIssueAndCodeCoverageEvolutionNegative
     * () throws Exception {
     * when(config.canApprovePullRequest()).thenReturn(true);
     * 
     * SonarQubeIssuesReport report = mock(SonarQubeIssuesReport.class);
     ...
     * times(1)).postCoverageReport(eq(pr), eq(diffReport), (StashClient)
     * Mockito.anyObject()); verify(stashRequestFacade,
     * times(0)).approvePullRequest(eq(pr), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(1)).resetPullRequestApproval(eq(pr),
     * (StashClient) Mockito.anyObject()); }
     */

    /*
     * @Test public void
     * testExecuteOnWithPullRequestApprovalAndNewIssueAndCodeCoverageEvolutionPositive
     * () throws Exception {
     * when(config.canApprovePullRequest()).thenReturn(true);
     * 
     * SonarQubeIssuesReport report = mock(SonarQubeIssuesReport.class);
    ...
    /*
     * @Test public void testExecuteOnWithoutPullRequestApproval() throws
     * Exception { when(config.canApprovePullRequest()).thenReturn(false);
     * 
     * SonarQubeIssuesReport report = mock(SonarQubeIssuesReport.class);
     * when(report.countIssues()).thenReturn(0);
     * when(stashRequestFacade.extractIssueReport(projectIssues,
     ...
     * verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr),
     * eq(report), eq(diffReport), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(1)).postCoverageReport(eq(pr),
     * eq(diffReport), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(1)).postAnalysisOverview(eq(pr),
     * eq(STASH_ISSUE_THRESHOLD), eq(report), (StashClient)
     * Mockito.anyObject()); verify(stashRequestFacade,
     * times(0)).approvePullRequest(eq(pr), (StashClient) Mockito.anyObject());
     ...
     * (StashClient) Mockito.anyObject()); }
     * 
     * @Test public void testExecuteOnWithCodeCoverageSecurityAsNone() throws
     * Exception {
     * when(stashRequestFacade.getCodeCoverageSeverity()).thenReturn(StashPlugin
     * .SEVERITY_NONE);
     * 
     * SonarQubeIssuesReport report = mock(SonarQubeIssuesReport.class);
     * when(report.countIssues()).thenReturn(10);
     ...
     * myJob.executeOn(project, context);
     * 
     * verify(stashRequestFacade,
     * times(0)).getCoverageReport(eq(project.getKey()), eq(context),
     * eq(inputFileCacheSensor), anyString(), (SonarQubeClient)
     * Mockito.anyObject());
     * 
     * verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr),
     * eq(report), eq(diffReport), (StashClient) Mockito.anyObject());
     * verify(stashRequestFacade, times(1)).postCoverageReport(eq(pr),
     * (CoverageIssuesReport) Mockito.anyObject(), eq(diffReport), (StashClient)
     * Mockito.anyObject()); verify(stashRequestFacade,
     * times(1)).postAnalysisOverview(eq(pr), eq(STASH_ISSUE_THRESHOLD),
     * eq(report), (CoverageIssuesReport) Mockito.anyObject(), (StashClient)
     * Mockito.anyObject()); }
     * 
     * @Test public void testExecuteOnWithCodeCoverageSecurityAsInfo() throws
     * Exception {
     ...
     * 
     * verify(stashRequestFacade,
     * times(1)).getCoverageReport(eq(project.getKey()), eq(context),
     * eq(inputFileCacheSensor), anyString(), (SonarQubeClient)
     * Mockito.anyObject());
     * 
     * verify(stashRequestFacade, times(1)).postSonarQubeReport(eq(pr),
     ...
     * Mockito.anyObject()); }
     * 
     * @Test public void testExecuteOnWithoutAnalysisComment() throws Exception
